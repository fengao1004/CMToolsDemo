...
public class MediaFile {
	...
	private static String returnObject[] = null;
	...
	public static final int FILE_TYPE_MOV = 25;
	 ...
	 * @author 任育伟
	 */
	public static String convertTimeFormat(int minuteParam) {
		...
		if (minuteParam <= 0) {
			...
			if (minute < 60) {
				...
				timeString = hourStr + unitFormat(minute) + ":"
						+ unitFormat(second);
			}
		}
		return timeString;
	...
	public static String[] getPlayTime(String playFilePath) {
		...
		if (playFile.exists()) {
			...
			try {
				 ...
				 int second = player.getDuration() / 1000;
             	 String playTime = MediaFile.convertTimeFormat(second);
 				 returnObject[0] = String.valueOf(second);
 				 returnObject[1] = playTime;
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
